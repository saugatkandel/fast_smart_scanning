...
class Sample:
    ...
    @staticmethod
    def _perform_measurements_from_recon(mask, measurement_info, recon_image, new_idxs):
        """
        ...
        measurement_info.measured_idxs = np.concatenate((measurement_info.measured_idxs,
                                                         new_idxs), axis=0)
    ...
    def _get_limited_update_locations(self,
        ...
        if self.params_erd.calculate_full_erd_per_step or \
                self.iteration % self.params_erd.full_erd_recalculation_frequency == 0:
    ...
    def reconstruct_and_compute_erd(self):
        ...
        unmeasured_idxs = self._get_limited_update_locations(self.mask.copy(),
                                                                 deepcopy(self.measurement_info))
    ...
    def _reconstruct_and_compute_erd_from_recon(self, mask, measurement_info, recon_image, erd):
        ...
        unmeasured_idxs = self._get_limited_update_locations(mask.copy(),
                                                             deepcopy(measurement_info))
        ...
        if self.params_erd.model_type == 'slads-net':
            poly_features = compute_poly_features(self.params_sample, recon_image,
                                                  unmeasured_idxs, neighbors,
                                                  self.params_erd.feat_distance_cutoff,
                                                  self.params_erd.feature_type)
    ...
    def _iterative_find_new_idxs_from_recon(self):
        ...
        while True:
            ...
            mask, measurement_info, new_values = self._perform_measurements_from_recon(mask,
                                                                                       measurement_info,
                                                                                       recon_image,
                                                                                       new_idx)
