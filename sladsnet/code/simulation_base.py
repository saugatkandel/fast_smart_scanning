...
class SimulatedSample(Sample):
    ...
    def reconstruct_and_compute_erd(self):
        ...
        self.RD = self._compute_rd(self.mask, self.measurement_info, self.neighbors, self.RDPP, self.ERD,
                                   unmeasured_idxs)
    ...
    def _compute_rd(self, mask, measurement_info, neighbors, rdpp, rd, update_idxs):
        ...
        indices = [np.where(np.all(measurement_info.unmeasured_idxs == idx, axis=1))[0][0]
